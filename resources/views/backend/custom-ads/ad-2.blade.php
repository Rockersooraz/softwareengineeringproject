                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="panel-body">
                                ...
                                <div class="box">
            <div class="box-header">
              <h3 class="box-title">Custom-ad-2</h3>
              <form class="form form-submit" method="post" action="{{route('backend.custom-adsii.store')}}"
             ...
             class="table example   table-responsive editable_table table-striped table-hover">
                ...
                <tbody>
                    @foreach($ads as $count => $key)
                             <tr>
                                    <td>{{++$count}}</td>
                                            <td class="w-80"><img src="{{ asset('/images/Customads-2'.'/'.$key->filename) }}" alt="" class="avatar avatar-sm"></td>
                                            <td>{{$key->title}}</td>
                                            <td>
                                                <table>
                                                    <tr>
                     ...
                     <form id="delete-form-{{$key->id}}" method="post" action="{{route('backend.custom-adsii.destroy',$key->id)}}" style="display: none;">
                        {{csrf_field()}}
                        {{method_field('Delete')}}
                      </form>
                    <a onclick="
